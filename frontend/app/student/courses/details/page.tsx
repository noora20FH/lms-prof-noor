...
function StudentCourseDetailContent() {
  ...
  useEffect(() => {
    ...
    const loadCourseWeeks = async () => {
      try {
        ...
        setWeeks([]);
        setError(
          getErrorMessage(requestError, 'Gagal mengambil daftar week course.')
        );
      } finally {
        setIsLoading(false);
      }
  ...
  if (isLoading) {
    return <div className="p-8 text-center">Loading course detail...</div>;
  }
        ...
        <div className="divide-y divide-gray-200 overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
          {weeks.map((week) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`flex h-12 w-12 items-center justify-center rounded-2xl font-semibold text-xl ${
                    week.is_locked
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {week.materials_count} materi dan {week.assignments_count}{' '}
                    assignment
                  </p>
                  {week.is_locked && (
          ...
          {weeks.length === 0 && (
            <div className="p-8 text-center text-gray-500">
              Belum ada week pada course ini.
            </div>
          )}
        </div>
      </div>
...
export default function StudentCourseDetailPage() {
  return (
    <Suspense
      fallback={<div className="p-8 text-center">Loading course detail...</div>}
    >
