...
export type EditableCourseData = {
  id: string;
  title: string;
  description: string;
...
const getInitialCourseForm = (
  ...
  totalStudents: course ? String(course.totalStudents) : "",
...
export default function EditCourseModal({
  ...
  const [courseForm, setCourseForm] = useState<CourseFormState>(
    getInitialCourseForm(course)
  );

  useEffect(() => {
    if (isOpen) {
      setCourseForm(getInitialCourseForm(course));
    }
  }, [isOpen, course]);
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5 px-6 py-6">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div className="space-y-2">
              <label
                htmlFor="edit-course-total-students"
                className="text-sm font-medium text-gray-700"
              >
                Total Mahasiswa
              </label>
              <input
                id="edit-course-total-students"
                type="number"
                min={1}
